...
test("agent verification works during profile loading, offers verification and retries after another-tab verification", async () => {
  const h = hooks();
  let verified = false, fail = false;
  ...
  const page = await load("app/become-agent/page.tsx", {
    ...
    "@/services/emailVerification": { reloadVerifiedRecruitmentUser: async () => { if (fail) throw new Error("offline"); return verified; } }
  }, { fetch: async () => ({ ok: true, json: async () => ({ commissionRate: .01 }) }) });
  ...
  assert.equal(tree.props.title, "Unable to check email verification");
  ...
  assert.ok(nodes(tree).some(n => n.type === "form"), "Verified user reaches activation terms");
});
