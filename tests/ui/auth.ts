...
export const user={uid:profile.uid,displayName:profile.displayName,email:profile.email,emailVerified:sessionStorage.getItem("fixture.verification")!=="pending",reload:async()=>{if(location.pathname==="/verify-email" && ++verificationChecks>1){user.emailVerified=true;sessionStorage.removeItem("fixture.verification");}},getIdToken:async()=>"local-ui-fixture"};
