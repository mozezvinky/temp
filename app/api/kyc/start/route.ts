...
import { localVerificationUploadExists, saveLocalVerificationUpload, shouldUseLocalVerificationStorage } from "@/lib/verification-local-storage";
...
async function saveVerificationUpload(userId: string, submissionId: number, name: string, file: File) {
  const path = `verification/${userId}/${submissionId}/${name}`;
  const buffer = Buffer.from(await file.arrayBuffer());
  if (shouldUseLocalVerificationStorage()) {
    await saveLocalVerificationUpload(path, buffer);
    return path;
  }
...
async function assertVerificationUploadExists(path: string) {
  if (shouldUseLocalVerificationStorage() && await localVerificationUploadExists(path)) return;
  const candidates = firebaseStorageBucketCandidates();
  for (const bucketName of candidates) {
    try {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isMultipart) {
      ...
      idFrontUrl = String(input.idFrontUrl ?? "").trim();
      idBackUrl = String(input.idBackUrl ?? "").trim();
      selfieWithIdUrl = String(input.selfieWithIdUrl ?? "").trim();
    }
    ...
    return NextResponse.json({ error: message }, { status: message.includes("already attached") ? 409 : authErrorStatus(error) });
  }
}
