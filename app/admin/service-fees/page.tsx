      ...
      {payments.length ? payments.map(payment => (
        <Card key={payment.id}>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button type="button" disabled={busy === payment.id || payment.status === "approved"} onClick={() => void review(payment.id, "approve")}>Approve Payment</Button>
            <Button type="button" variant="danger" disabled={busy === payment.id || payment.status === "approved"} onClick={() => void review(payment.id, "reject")}>Reject Payment</Button>
          </div>
        </Card>
      )) : <EmptyState title="No service fee payments" body="Worker submissions will appear here for verification." />}
    </div>
  );
}
