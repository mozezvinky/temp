    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-[#FFFBFF]">Completed requests</h1>
        </div>
      </div>
      ...
      {requests.length ? (
        <div className="grid gap-4">
          {requests.map(application => (
            <Card key={application.id} id={`completion-request-${application.id}`} className={selectedApplicationId === application.id ? "ring-2 ring-bone" : ""}>
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <h2 className="text-xl font-black text-[#FFFBFF]">{application.jobTitle ?? "Completed job request"}</h2>
                  <span className="mt-3 inline-flex rounded-full border border-sky-300/40 bg-sky-400/20 px-3 py-1 text-sm font-black text-sky-100">Completion requested</span>
                </div>
              ...
              <div className="mt-5 rounded-xl border border-[#4A463F] bg-[#2A2A2B] p-4">
                <p className="text-lg font-black text-[#FFFBFF]">{application.workerName ?? "Worker"}</p>
