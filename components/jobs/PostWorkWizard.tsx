  ...
  async function post() {
    ...
    try {
      await createJob(profile.id, {
        ...
        duration: durationLabel(Number(draft.timeline), draft.timelineUnit),
        durationValue: draft.timeline,
        durationUnit: draft.timelineUnit,
        durationHours: durationToHours(Number(draft.timeline), draft.timelineUnit),
        ...
        {step === 2 && (
          <form onSubmit={nextFromDetails} className="popup-form mt-6 grid gap-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="temp-label">Budget<input value={draft.budget} onChange={event => setValue("budget", event.target.value)} required type="number" min={50} placeholder="KES" className="temp-input p-3 outline-none" /></label>
              ...
              <label className="temp-label sm:col-span-2">Pay type<select value={draft.payType} onChange={event => setValue("payType", event.target.value as Draft["payType"])} className="temp-input p-3 outline-none"><option value="fixed">Fixed pay</option><option value="timeline">Per timeline</option></select></label>
            </div>
